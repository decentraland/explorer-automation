...
function Invoke-Recorder([string[]]$Arguments, [string]$LogName, [int]$TimeoutSeconds = 30) {
    $command = Start-Process $wpr -WindowStyle Hidden -PassThru -ArgumentList $Arguments `
        -RedirectStandardOutput (Join-Path $OutputDirectory ($LogName + '.log')) `
        -RedirectStandardError (Join-Path $OutputDirectory ($LogName + '-error.log'))
    try {
