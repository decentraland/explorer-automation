...
test.describe('@cross Explorer wallet login', () => {
  ...
  test('wallet login via web reaches the requests page and lands client in-world', async ({
    ...
    //
    // SCOPED to the verify-sign-in card, NOT the whole document: a
    // document-wide scan would also match unrelated numeric content (counters,
    // prices, timestamps, "0 notifications") and silently return the wrong
    // value. We anchor on the approve button (already visible above) and climb
    // to the smallest ancestor that actually holds a numeric leaf — that
    // subtree is the pairing-code card, which contains both the code and the
    ...
    const readWebCode = (): Promise<string | null> =>
      page.evaluate(() => {
        const approve = document.querySelector('[data-testid="verify-sign-in-approve-button"]')
        if (!approve) return null

        // Digit-only leaves within `root`, concatenated per parent. Catches
        // both single-element codes (`<div>78</div>`) and per-digit codes
        // (`<div>1</div><div>3</div>`). Require >= 2 digits: the pairing code
        // is always multi-digit (e.g. "96"), so a length-2 floor rejects a
        // stray single digit ("0", "1") that could survive within the card.
        const codeWithin = (root: Element): string | null => {
          ...
          return (
            Array.from(groups.values())
              .map(d => d.join(''))
              .find(s => s.length >= 2) ?? null
          )
        }

        // Climb from the approve button to the nearest ancestor that contains a
        // numeric leaf (cap the walk so a code-less page can't loop to <html>).
        for (let scope = approve.parentElement, i = 0; scope && i < 6; scope = scope.parentElement, i++) {
          const code = codeWithin(scope)
          if (code) return code
        }
        return null
      })
