...
└── tests/
    ...
    │       ├── request-page.spec.ts              # @web @auth — RequestPage eth_sendTransaction broker
...
### Specs track the auth release, not the current prod deploy

These specs run against the **deployed** dapp, and auth-site rolls out `.zone` → `.today` → `.org`, so a freshly-merged auth change is live on `.zone` days before `.org`. Specs are written against the auth **release** (`decentraland/auth` `main` / its newest tag), not against whatever `.org` happens to serve today. A spec covering just-merged behaviour is therefore expected to fail on `.org` until the rollout reaches it — validate it against the environment that has the build (`WEB_BASE_URL=https://decentraland.zone`, which needs the CF Access service token) and let `.org` go green when the deploy lands.

Don't add runtime version gates or `test.skip`s to paper over a pending rollout: they hide whether the deploy actually shipped the behaviour. Note the required auth-site version in the spec's header comment instead. Check what a host is serving with the `ROLLOUTS` blob in any auth page's HTML head (`"prefix":"@dcl/auth-site","version":"…"`).
...
- `AUTH_SERVER_URL` — RequestPage tests broker `eth_sendTransaction` requests (and, in the retired-sign-in negative test, a `dcl_personal_sign` request the dapp is expected to reject) through this. Auto-derived as `auth-api.<host>` from `WEB_BASE_URL` (so a `.today` dapp run talks to `auth-api.decentraland.today`); set this only to point at a non-paired auth-api host.
...
- Don't re-cover what `decentraland/auth`'s own `e2e/` suite already tests against mocked services. Its Explorer journeys (`explorer-metamask-flow.spec.ts`, `explorer-social-flow.spec.ts`) already drive the `?flow=deeplink` identity handoff — auto-connect, `POST /identities`, the `open?signin=<id>` deep link, and the "never recovers a backing request" invariant. This suite's value is what mocks can't give: the **deployed** dapp against **real** auth-api / catalysts / Thirdweb. Check `auth/e2e/tests/` before adding a spec, and add one here only when running it against live services is the point.
