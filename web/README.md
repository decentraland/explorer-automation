...
| `@web`                   | [tests/auth/specs/request-page.spec.ts](tests/auth/specs/request-page.spec.ts)                         | `eth_sendTransaction` via the **RequestPage** flow (`/auth/requests/<id>`)                                                                                              | mocked web3 wallet                             | exercises the desktop-handoff signature broker via `auth-api.decentraland.org`                                                                                  |
| `@web`                   | [tests/auth/specs/request-page-failure.spec.ts](tests/auth/specs/request-page-failure.spec.ts)         | the retired `dcl_personal_sign` sign-in shows "update your app" and signs nothing                                                                                       | mocked web3 wallet                             | needs auth-site >= 5.0.0 on the target host (auth#437) — validate on `.zone` first                                                                              |
...
| `@builder`               | [tests/builder/specs/collection-management.spec.ts](tests/builder/specs/collection-management.spec.ts) | create / rename / copy-URN / delete collection                                                                                                                          | shared wallet (`WALLET_A`, ephemeral fallback) | off-chain (builder-server writes only); tracked cascade cleanup of exactly the collections each test creates                                                    |
| `@builder`               | [tests/builder/specs/item-upload.spec.ts](tests/builder/specs/item-upload.spec.ts)                     | serial flow on ONE shared collection: create → upload wearable GLB → emote GLB (+ thumbnail screenshot) → smart-wearable ZIP + video → 3MB-limit negative → delete item | shared wallet (`WALLET_A`, ephemeral fallback) | off-chain; single shared browser context (`describe.serial`); stale-sweep on start + cascade delete on end; fixture files under `tests/builder/fixtures/files/` |
| `@builder`               | [tests/builder/specs/access-control.spec.ts](tests/builder/specs/access-control.spec.ts)               | non-committee wallet blocked from `/curation`; stranger can't see foreign unpublished collection; empty state for a fresh account                                       | ephemeral wallets                              | off-chain negative + isolation guards; second wallet via `newWalletContext`                                                                                     |
| `@builder`               | [tests/builder/specs/sign-in-required.spec.ts](tests/builder/specs/sign-in-required.spec.ts)           | signed-out visitor is prompted to sign in on `/collections`                                                                                                             | none                                           | POM-only fixture flavor (no wallet mock)                                                                                                                        |
| `@builder @on-chain`     | [tests/builder/specs/publish-lifecycle.spec.ts](tests/builder/specs/publish-lifecycle.spec.ts)         | serial lifecycle: publish (MANA fee) → committee approval (rescue/deploy/enable) → item update → re-review → curation rejection → resubmission                          | `WALLET_A` (creator AND dev curator)           | `builder-onchain` project (manual dispatch; ~1 MANA + permanent dev collection per run); leaves the collection id for `BUILDER_TEST_PUBLISHED_COLLECTION_ID`    |
| `@builder-api`           | [tests/builder/specs/authorization.spec.ts](tests/builder/specs/authorization.spec.ts)                 | builder-server authorization regression (read-leak + unauthorized-mutation axes)                                                                                        | in-process auth chain (no browser)             | LOCAL builder-server only (`BUILDER_API_URL` interlock); self-skips when unconfigured                                                                           |
...
npm run test:builder:offchain  # @builder UI specs (ephemeral wallets, no .env needed)
npm run test:builder:onchain   # @on-chain builder specs (serial; needs BUILDER_* wallets)
npm run test:builder:api       # @builder-api authorization spec (local builder-server only)
